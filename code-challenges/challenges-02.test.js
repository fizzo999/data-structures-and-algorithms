...
const forLoopTwoToThe = (arr) => {
  ...
  for (let i=0; i < arr.length; i++) {
   resultsArray4.push(Math.pow(2, arr[i]));  
  }
...
const forEachTwoToThe = (arr) => {
  const resultsArray4 = [];
    arr.forEach(num => resultsArray4.push(Math.pow(2, num)));  
  return resultsArray4;
};
...
const extractAbilities = (arr) => {
  const resultsArr9 = arr.map(sonrlax => sonrlax.ability.name);
...
const extractStats = (arr) => {
  var newObj = {name: '', total: 0};
  const reultsArr10 = arr.map(inhabitantStat => {
    return {
      name: inhabitantStat.stat.name, 
      total: inhabitantStat.effort + inhabitantStat.baseStat
    }
  });
  console.log('here is 10:', reultsArr10);
return reultsArr10
};
